...
int ReadPPM(char * fname,struct IMG * img)
{
	...
	//img->img=(unsigned char *)realloc(img->img,img->sx*img->sy);
	if(img->img == NULL)
	{
	    printf("分配内存失败!\n");
	    exit(-1);
...
int TemplateFun(struct Template * ptml,int * array,int x,int y,char * name)
{
    return 0;
}
